 ...
 *
 * <p>The failure this guards against is a buffer delivered twice in place of its successor: the blob keeps its exact
 * length and every other block is correct, so no length or byte-count check can see it. Only comparing what arrived
 * against what was sent will catch it.
 *
 * <p>This class is excluded from the {@code test} task and runs under {@code azureUploadIntegrityTest}, which sets a
 * small send-queue prefetch. {@code reactor.netty.channel.MonoSend} requests {@code maxPrefetchSize} buffers up front
 * and refills when demand falls to half of it, so a small value takes the refill path constantly instead of once per
 * 64 buffers, and the failure reproduces within a handful of uploads rather than being vanishingly rare:
 * <pre>
 * ./gradlew :modules:repository-azure:azureUploadIntegrityTest
 * </pre>
 * Raise {@code -Dtests.upload.iterations} to hunt harder.
 */
...
public class AzureUploadIntegrityTests extends ESTestCase {
    ...
    /** The upload buffer size {@link AzureBlobStore} reads with, and so the granularity a substituted block aligns to. */
    private static final int BUFFER_SIZE = Math.toIntExact(ByteSizeValue.of(64, ByteSizeUnit.KB).getBytes());

    /** Uploads per thread. The fault is a thread race, so detection is probabilistic in the number of uploads. */
    private static final int ITERATIONS = Integer.getInteger("tests.upload.iterations", 12);

    /** Buffers per blob. Must exceed the send queue's prefetch for the refill path to be exercised at all. */
    private static final int BUFFERS_PER_BLOB = Integer.getInteger("tests.upload.buffersPerBlob", 600);
    ...
    /**
     * Concurrent uploads with no injected failures, so nothing is retried. The fault does not need a retry to occur,
    ...
    private class Fixture {
         ...
         * shape to look for - a block holding a byte-exact copy of a nearby earlier block - when that is what it is.
         */
        ...
        httpServer.createContext("/" + ACCOUNT + "/" + CONTAINER, exchange -> {
            try {
                ...
                if (fixture.shouldFail(blobName)) {
                    ...
                    // Discard as we go: these bodies are tens of megabytes and several are in flight at once.
    ...
    /** Reads a request body to the end without retaining it. */
    private static void discard(InputStream in) throws IOException {
        final byte[] scratch = new byte[BUFFER_SIZE];
